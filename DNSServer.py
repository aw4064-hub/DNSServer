...
def encrypt_with_aes(input_string, password, salt):
    key = generate_aes_key(password, bytes(salt, 'utf-8'))
    f = Fernet(key)
        ...
        try:
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                if qtype == dns.rdatatype.MX:
                    ...
                    mname, rname, serial, refresh, retry, expire, min = answer_data # What is the record format? See dns_records dictionary. Assume we handle @, Class, TTL elsewhere. Do some research on SOA Records
                    rdata = SOA(dns.rdataclass.IN, dns.rdatatype.SOA, mname, rname, serial, refresh, retry, expire, min) # follow format from previous line
